...
## Inicio obligatorio
1. Leer este archivo, `_project_intelligence/STATE.json`, `HANDOFF.md`, `MEMORY.md` y `_project_intelligence/BACKLOG.md`.
...
5. Crear recibo BOOTSTRAP del run con agente, alias de sesión, base y aprendizajes recuperados. El recibo no aumenta autoridad.
...
- Ejecutar `python3 -m unittest discover -s tests -p 'test_governance.py'` y `python3 tools/project_control.py check`.
...
- Actualizar STATE (única autoridad de estado), PROGRESS, HANDOFF, graph.json y controles afectados. Ejecutar `python3 tools/project_control.py refresh` y después `check`.
