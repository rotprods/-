...
const CLAIM: String = "CLM-ORIGIN-MEGA-ATRIO-001"
const MODEL_REVISION: int = 2
const RING_Y: float = 4.0
const RING_COLLISION_HEIGHT: float = 0.648
const RING_COLLISION_WIDTH: float = 4.6
const BRIDGE_COLLISION_HEIGHT: float = 0.55
const BRIDGE_COLLISION_WIDTH: float = 4.1
...
func _run() -> void:
	...
	var out: FileAccess = FileAccess.open("res://evidence/origin-atrio-surrogate.json", FileAccess.WRITE)
	if out == null:
		push_error("Could not create surrogate receipt")
		quit(2)
		return
...
func _build_collision_surrogate() -> void:
	for radius_value in [35.0, 24.0, 13.0]:
		var radius: float = float(radius_value)
		for center_degree_value in [0.0, 120.0, 240.0]:
			var center_degrees: float = float(center_degree_value)
			_add_ring_sector(radius, deg_to_rad(center_degrees), 18)

	for angle_degree_value in [0.0, 120.0, 240.0]:
		var angle_degrees: float = float(angle_degree_value)
		var angle: float = deg_to_rad(angle_degrees)
		_add_bridge_span(angle, 15.5, 21.5)
		_add_bridge_span(angle, 26.5, 32.5)

	_add_dodecagonal_node("COL_ORG_ATR_NODE_SAFE", Vector3(24.0, RING_Y, 0.0), 3.72, 0.58)
	var valve_angle: float = deg_to_rad(120.0)
	_add_dodecagonal_node(
		"COL_ORG_ATR_NODE_VALVE",
		Vector3(24.0 * cos(valve_angle), RING_Y, 24.0 * sin(valve_angle)),
		3.72,
		0.58
	)

	# Route-aperture pylons: same horizontal proxy dimensions and placement as rev2 generator.
	var aperture_angle: float = deg_to_rad(60.0)
	var radial: Vector3 = Vector3(cos(aperture_angle), 0.0, sin(aperture_angle))
	var tangent: Vector3 = Vector3(-sin(aperture_angle), 0.0, cos(aperture_angle))
	var aperture_base: Vector3 = radial * 35.0
	for side_value in [-1.0, 1.0]:
		var side: float = float(side_value)
		var center: Vector3 = aperture_base + tangent * (side * 3.0)
		center.y = RING_Y + 2.55
		_add_box(
			"COL_ORG_ATR_ROUTE_APERTURE_PYLON_%s" % side,
			center,
			Vector3(0.95, 5.0, 1.40),
			-aperture_angle
		)

func _add_ring_sector(radius: float, center_angle: float, segments: int) -> void:
	var span: float = deg_to_rad(108.0)
	var step: float = span / float(segments)
	for i in range(segments):
		var angle: float = center_angle - span * 0.5 + (float(i) + 0.5) * step
		var chord: float = 2.0 * radius * sin(step * 0.5) * 1.06
		var center: Vector3 = Vector3(radius * cos(angle), RING_Y, radius * sin(angle))
		_add_box(
...
func _add_bridge_span(angle: float, r0: float, r1: float) -> void:
	var midpoint_radius: float = (r0 + r1) * 0.5
	var length: float = r1 - r0
	var center: Vector3 = Vector3(midpoint_radius * cos(angle), RING_Y, midpoint_radius * sin(angle))
	_add_box(
...
func _add_dodecagonal_node(name_: String, center: Vector3, radius: float, height: float) -> void:
	...
	for i in range(12):
		...
		points.append(Vector3(radius * cos(angle), -height * 0.5, radius * sin(angle)))
		points.append(Vector3(radius * cos(angle), height * 0.5, radius * sin(angle)))
...
func _walk_to(target: Vector3, max_frames: int = 240) -> bool:
	for _i in range(max_frames):
		...
		var direction: Vector3 = delta_xz.normalized()
...
func _case_outer_ring_sector() -> bool:
	var start_angle: float = deg_to_rad(-40.0)
	var reset_ok: bool = await _reset_player(Vector3(35.0 * cos(start_angle), 0.0, 35.0 * sin(start_angle)))
	if not reset_ok:
		return false
	for degree_value in [-30.0, -20.0, -10.0, 0.0, 10.0, 20.0, 30.0, 40.0]:
		var degrees: float = float(degree_value)
		var angle: float = deg_to_rad(degrees)
		var walked: bool = await _walk_to(Vector3(35.0 * cos(angle), 0.0, 35.0 * sin(angle)))
		if not walked:
			return false
	return true

func _case_radial_bridge_chain() -> bool:
	var reset_ok: bool = await _reset_player(Vector3(34.0, 0.0, 0.0))
	if not reset_ok:
		return false
	for radius_value in [31.0, 28.0, 25.0, 22.5, 19.0, 16.0, 14.0]:
		var radius: float = float(radius_value)
		var walked: bool = await _walk_to(Vector3(radius, 0.0, 0.0))
		if not walked:
			return false
	return true

func _case_middle_ring_node_zone() -> bool:
	var start_angle: float = deg_to_rad(-14.0)
	var reset_ok: bool = await _reset_player(Vector3(24.0 * cos(start_angle), 0.0, 24.0 * sin(start_angle)))
	if not reset_ok:
		return false
	for degree_value in [-8.0, -3.0, 0.0, 3.0, 8.0, 14.0]:
		var degrees: float = float(degree_value)
		var angle: float = deg_to_rad(degrees)
		var walked: bool = await _walk_to(Vector3(24.0 * cos(angle), 0.0, 24.0 * sin(angle)))
		if not walked:
			return false
	return true
